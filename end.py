...
def pre1(message):
    global zapros
    zapros += f'белый фон на фоне человека в {message.text} одежде, '
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('спортивный')
    item2 = types.KeyboardButton('классический')
...
def pre2(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('верхняя одежда-пальто')
    item2 = types.KeyboardButton('верхняя одежда-футболка')
    item3 = types.KeyboardButton('нижняя одежда-шорты')
    markup.row(item1, item2, item3)
    bot.send_message(message.chat.id, 'Выберите уровень одежды', reply_markup=markup)
...
def pre3(message):
    ...
    if message.text == "верхняя одежда-пальто":
        ...
        bot.send_message(message.chat.id, 'Выберите тип одежды', reply_markup=markup)
