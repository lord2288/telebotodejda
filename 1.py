...
def echo_all(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    options = ["Начать генерация", "Tex.поддежка"]
...
def handle_main_menu(message):
    if message.text == "Начать генерация":
        ...
        bot.send_message(message.chat.id, key.email)
        show_main_menu(message)
    else:
...
def choose_style(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    categories = ['Верхняя одежда', 'Низ', 'Обувь']
...
def choose_item(message):
    if message.text == 'Готово':
        ...
        bot.send_message(message.chat.id, 'Выберите элемент одежды или нажмите "Готово" для завершения.', reply_markup=markup)
...
def finalize_or_add(message):
    ...
    # else:
    #     request_data['color'] = message.text
